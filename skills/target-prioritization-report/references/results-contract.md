...
### Part2 Table: `geo_pct_rank` Column (Combo Scoring)

Part2_Combo_Analysis.md tables include a `geo_pct_rank` column that represents the geometric mean of individual gene disease ranks, converted to percentile space. This provides an expected combo performance metric independent of the intersection query result.

**Usage rule:**
- **ULTRA combo scoring:** prefer `geo_pct_rank × 100` as the combo ULTRA disease score
- **BioBridge combo scoring:** use the direct combo `pct_rank` (NOT `geo_pct_rank`)
- **Fallback (either method):** if no Part2 value available, use mean of member-gene scores

**Part2 table column layouts vary by indication but always include:**
- First column: combo name (format: `GENE1+GENE2[+GENE3...]`)
- A `geo_pct_rank` column (float, 0-1 scale)
- A `Delta` column (combo pct_rank minus geo_pct_rank; classification: SYNERGY/NEAR-ADD/DILUTION)

**Parser guidance:**
- Find the table header row containing "geo_pct_rank"
- Identify the column index dynamically (position varies)
- Extract combo name from column 0 (strip `**` markdown bold markers)
- Store both raw and sorted key variants (e.g., `TYK2+JAK1` and `JAK1+TYK2`) for robust lookup
...
| Single-cell coexp | `results/sc_coexp_<indication>/**.html` + `results/sc_coexp_<indication>/**.md` | Plotly exports + interpretation (pipeline-only, NOT embedded in report) |
| CI dashboard | `results/ci_<indication>/*.html` with `<script id="data">` JSON | Competitive Intelligence |

### Single-Cell AUCell (from omicsoft sc_target_result) — pipeline output only

Per-combo single-cell outputs from `sc_target_analysis.py`. These are generated by the
pipeline but **NOT embedded in the HTML report** (they would inflate it 40-80+ MB). View
them directly on disk or via `all_plots_interactive.html`.

- Path pattern: `results/omicsoft_full_<indication>/{combo_name}/sc_target_result/{study_name}/`
- Artifacts per study:
  - `dotplot_interactive.html` — Plotly gene expression + AUCell score dotplots
  - `umap_interactive.html` — Plotly UMAP/embedding plots (colored by condition, cell type, gene, score)
  - `heatmap_interactive.html` — Plotly correlation heatmaps (global + per-cell-type)
  - `{study}_report.pdf` — Static PDF report (all matplotlib figures)
  - `all_plots_interactive.html` — Searchable index of all plots
- Studies: jhu, rieder, umcg_fibroblast, otar, tacolny, pf_lung, ssc_lung, hs_skin, ibd_atlas, umcg_change
- Regeneration: `scripts/run_sc_study_all_combos.py --filter-combos <slugs>` to run just specific combos
...
| `commercial` | `results/commercial_<indication>/commercial_scores.json` | JSON | `commercial_score`, `confidence`, `components{market,competitive,strategic}`, `narrative_summary`, `citations` |
| `ukbppp` | `test/genetics_ukbppp/*_<icd10>_ukbppp.tsv` | TSV | Per-gene disease association scores (FDR, direction, model count) |
...
- Display name: `GENE1 + GENE2 (+ GENE3)` (spaces around `+`)
- File slug: `GENE1_GENE2[_GENE3]` (underscores, unchanged)
...
## Score Tables (DEG + Signature)

Each combo directory under `test/omicsoft_full/<COMBO_SLUG>/` contains:
- `target_score_table.csv` — DEG aggregate scores per gene (Gene, total_score, n_comparisons, n_significant, disease_vs_normal_score, nonresp_vs_resp_score, resp_vs_nonresp_score, approach). `approach` is "agonist" or "antagonist".
- `signature_score_table.csv` — GSEA NES-based scores, ONE row per signature (Signature, total_score, n_comparisons, n_significant, disease_vs_normal_score, nonresp_vs_resp_score, resp_vs_nonresp_score, associated_target, approach). Uses same percentile-rank scoring as target_score_table but with NES gated by FDR instead of log2fc gated by padj.

## ARCHS4 Tissue Expression (NEW)

Located at `test/archs4/`:
- `{combo_slug}_boxplot.html` — Plotly boxplot of tissue-specific expression
- `{combo_slug}_heatmap.html` — Plotly heatmap of tissue correlations

## CellXGene Single-Cell Coexpression (NEW)

Located at `test/cellxgene/{celltype}/`:
- `{celltype}_{combo_slug}_heatmap.html` — Per-celltype coexpression heatmap

Cell types are dynamic (only directories with data for the combo are shown).
