...
def calculate_disease_association(
    ...
    deg_max_score: Optional[float] = None,
    gene_approaches: Optional[Dict[str, str]] = None,
) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        deg_max_score: Dynamic max DEG score for normalization (computed from data).
                       If None, uses hardcoded default for backward compatibility.
        gene_approaches: Dict mapping gene names to approach ("antagonist" or "agonist").
    ...
    gene_approaches = gene_approaches or {}

    if is_combo:
        genes = data.get('genes', [])

        # DEG: Average individual genes (with approach-based transformation)
        deg_scores = []
        for gene in genes:
            ...
            if gene_key in data:
                ...
                if raw is not None:
                    approach = gene_approaches.get(gene, "antagonist")
                    norm = normalize_deg(raw, max_score=deg_max_score, approach=approach)
                    deg_scores.append(norm)
                    result['components'].setdefault('deg', {})[gene] = {
                        'raw': raw,
                        'normalized': norm,
                        'approach': approach,
                    }
        ...
        # Individual gene
        # DEG (with approach-based transformation)
        if 'deg' in data:
            raw = data['deg'].get('deg_score_raw')
            if raw is not None:
                # Get gene name from data for approach lookup
                gene_name = data.get('gene_name') or data.get('name')
                approach = gene_approaches.get(gene_name, "antagonist") if gene_name else "antagonist"
                norm = normalize_deg(raw, max_score=deg_max_score, approach=approach)
                components_normalized['deg'] = norm
                result['components']['deg'] = {
                    'raw': raw,
                    'normalized': norm,
                    'approach': approach,
                }
