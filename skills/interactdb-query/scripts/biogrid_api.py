...
# Default BioGRID API key - used when no key is provided
# Users can override by passing their own key or setting BIOGRID_API_KEY env var
DEFAULT_BIOGRID_API_KEY = "d3367ed24eeea8fe8718f4993ed63ec9"
...
class BioGRIDClient:
    def __init__(self, access_key: Optional[str] = None, base_url: str = BIOGRID_BASE_URL, timeout: int = 60):
        """
        ...
        access_key : str, optional
            BioGRID API key. If not provided, uses DEFAULT_BIOGRID_API_KEY.
            Can also be set via BIOGRID_API_KEY environment variable.
        base_url : str
            BioGRID REST API base URL
        timeout : int
            Request timeout in seconds (default: 60)
        """
        # Priority: provided key > env var > default key
        self.access_key = access_key or os.environ.get("BIOGRID_API_KEY") or DEFAULT_BIOGRID_API_KEY
                ...
                for gene, entrez, org in ((a, entrez_a, org_a), (b, entrez_b, org_b)):
                    ...
                    if gene not in neighbors:
                        ...
                        neighbors[gene] = NeighborRecord(
                            ...
                            publications=str(pubmed_id) if pubmed_id else None,
                            path=path_str,
                        )
                    else:
                        n = neighbors[gene]
                        ...
                        if pubmed_id and str(pubmed_id) not in (n.publications or ""):
                            n.publications = f"{n.publications}|{pubmed_id}" if n.publications else str(pubmed_id)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="BioGRID access key. Optional - uses default key if not provided."
    )
    parser.add_argument(
        "--tax-id", default="9606",
    ...
    # API key priority: CLI arg > env var > default
    api_key = args.api_key or os.environ.get("BIOGRID_API_KEY") or DEFAULT_BIOGRID_API_KEY
