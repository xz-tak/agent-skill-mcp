...
**BioGRID API Key:**
- BioGRID now works out-of-the-box with a built-in default API key
- No configuration required - just start querying
- Optionally set `BIOGRID_API_KEY` environment variable to use your own key
- Register at https://thebiogrid.org/ to obtain a free personal API key if desired
...
- When you want to query all three databases at once
...
# Initialize client (uses built-in default API key)
client = BioGRIDClient()  # Default timeout: 5 minutes
# Or provide your own key: BioGRIDClient(access_key="your_key_here")
...
| **BioGRID** | Genetic interactions, manual curation, multi-hop | Optional (built-in default) | ~5-10s (1-hop) | 2M+ interactions |
...
client = BioGRIDClient()  # Uses built-in default API key
neighbors = client.get_neighbors(
...
**BioGRID API Key** (Optional - built-in default available):
- BioGRID works out-of-the-box with a built-in default API key
- Optionally use your own key by setting the environment variable:
```bash
export BIOGRID_API_KEY="your_key_here"
```
- Register at https://thebiogrid.org/ to obtain a free personal API key if desired

**Note:** All three databases now work without any configuration. STRING and IntAct don't require API keys, and BioGRID uses a built-in default key.
