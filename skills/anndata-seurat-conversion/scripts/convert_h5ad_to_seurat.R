...
# -----------------------------------------------------------------------------
# 2b. Large Dataset Detection: Auto-switch to BPCells on-disk path
# Reticulate crashes on sparse matrices with >2.1B nnz (R 32-bit int limit).
# For datasets with >1M cells, use BPCells to read h5ad directly via HDF5.
# -----------------------------------------------------------------------------
BPCELLS_CELL_THRESHOLD <- 1e6
n_cells <- nrow(ad$obs)

if (n_cells > BPCELLS_CELL_THRESHOLD && requireNamespace("BPCells", quietly = TRUE)) {
  cat(sprintf("\n  *** LARGE DATASET DETECTED: %s cells > %s threshold ***\n",
              format(n_cells, big.mark = ","), format(BPCELLS_CELL_THRESHOLD, big.mark = ",")))
  cat("  Switching to BPCells on-disk path (avoids reticulate 2.1B nnz crash)\n\n")

  # Free the reticulate-loaded AnnData
  rm(ad)
  gc()

  # Source helper functions
  helpers_path <- file.path(dirname(sys.frame(1)$ofile %||% "."), "bpcells_helpers.R")
  if (!file.exists(helpers_path)) {
    helpers_path <- file.path(dirname(commandArgs(trailingOnly = FALSE)[
      grep("--file=", commandArgs(trailingOnly = FALSE))
    ] |> sub("--file=", "", x = _)), "bpcells_helpers.R")
  }
  ...
  # Determine which HDF5 group has counts
  h5 <- H5File$new(input_file, mode = "r")
  has_raw_counts_layer <- "layers" %in% names(h5) && "raw_counts" %in% names(h5[["layers"]])
  has_counts_layer <- "layers" %in% names(h5) && "counts" %in% names(h5[["layers"]])
  h5$close_all()

  counts_group <- if (has_raw_counts_layer) "layers/raw_counts"
                  else if (has_counts_layer) "layers/counts"
                  else "X"
  cat(sprintf("  Using HDF5 group: %s\n", counts_group))

  # Read matrix via BPCells (auto-transposes to genes x cells)
  cat("  Reading matrix via BPCells (on-disk, no RAM needed)...\n")
  mat <- open_matrix_anndata_hdf5(input_file, group = counts_group)
  cat(sprintf("  Matrix: %d genes x %d cells\n", nrow(mat), ncol(mat)))

  # Write to BPCells on-disk directory
  bpcells_dir <- file.path(dirname(output_file), "bpcells_counts")
  if (dir.exists(bpcells_dir)) unlink(bpcells_dir, recursive = TRUE)
  cat(sprintf("  Writing BPCells dir: %s\n", bpcells_dir))
  write_matrix_dir(mat, dir = bpcells_dir)
  mat_disk <- open_matrix_dir(bpcells_dir)
  cat(sprintf("  On-disk matrix ready: %d x %d\n", nrow(mat_disk), ncol(mat_disk)))

  # Read metadata via hdf5r (with Python fallback)
  cat("\n  Reading metadata via hdf5r...\n")
  if (exists("read_h5ad_metadata")) {
    meta_data <- read_h5ad_metadata(input_file)
  } else {
    # Inline fallback
    h5 <- H5File$new(input_file, mode = "r")
    obs <- h5[["obs"]]
    ...
    for (col in col_names) {
      tryCatch({
        ...
        if (inherits(item, "H5Group")) {
          ...
          vals <- item$read()
          if (length(vals) == length(obs_names)) meta_list[[col]] <- vals
        }
      }, error = function(e) NULL)
    }
    h5$close_all()
    meta_data <- as.data.frame(meta_list, stringsAsFactors = FALSE)
    rownames(meta_data) <- obs_names
  }
  cat(sprintf("  Metadata: %d rows x %d cols\n", nrow(meta_data), ncol(meta_data)))

  # Python fallback for metadata if too few columns read
  expected_min_cols <- 5
  if (ncol(meta_data) < expected_min_cols) {
    cat("  WARNING: hdf5r read too few columns. Trying Python fallback...\n")
    meta_tsv <- tempfile(fileext = ".tsv")
    export_py <- file.path(dirname(helpers_path), "export_metadata.py")
    if (file.exists(export_py)) {
      system2("python3", c(export_py, input_file, meta_tsv))
      if (file.exists(meta_tsv)) {
        meta_data <- read.table(meta_tsv, header = TRUE, sep = "\t",
                                 stringsAsFactors = FALSE, row.names = 1,
                                 quote = "", comment.char = "", check.names = FALSE)
        cat(sprintf("  Python fallback: %d rows x %d cols\n", nrow(meta_data), ncol(meta_data)))
        unlink(meta_tsv)
      }
    }
  }
  ...
  cat("\n  Creating Seurat v5 object (BPCells-backed)...\n")
  seurat_obj <- CreateSeuratObject(counts = mat_disk, meta.data = meta_data)
  cat(sprintf("  Seurat: %d genes x %d cells\n", nrow(seurat_obj), ncol(seurat_obj)))

  conversion_summary$counts_source <- paste0("BPCells on-disk (", counts_group, ")")
  conversion_summary$metadata_cols <- ncol(meta_data)

  # Save
  cat(sprintf("\n  Saving to %s...\n", output_file))
  saveRDS(seurat_obj, output_file)
  fsize <- file.info(output_file)$size / 1024^3
  cat(sprintf("  RDS size: %.2f GB\n", fsize))
  cat(sprintf("  BPCells dir: %s\n", bpcells_dir))
  cat("  NOTE: RDS references BPCells dir. Keep both together.\n")

  conversion_summary$status <- "SUCCESS"
  conversion_summary$layers <- c("BPCells on-disk counts")
  conversion_summary$reductions <- character()

  # Print summary and exit (skip the reticulate path below)
  cat("\n=============================================================================\n")
  cat("CONVERSION COMPLETE (BPCells large-dataset path)\n")
  cat("=============================================================================\n")
  cat(sprintf("  Status: %s\n", conversion_summary$status))
  cat(sprintf("  Cells: %s\n", format(conversion_summary$cells, big.mark = ",")))
  cat(sprintf("  Genes: %s\n", format(conversion_summary$genes, big.mark = ",")))
  cat(sprintf("  Counts: %s\n", conversion_summary$counts_source))
  cat(sprintf("  Metadata: %d columns\n", conversion_summary$metadata_cols))
  cat(sprintf("  Output: %s + %s\n", output_file, bpcells_dir))
